...
// import decodeOpaqueId from "@reactioncommerce/api-utils/lib/decodeOpaqueId";
// import encodeOpaqueId from "@reactioncommerce/api-utils/lib/encodeOpaqueId";
import { useSnackbar } from "@reactioncommerce/admin-core";
...
function ProductsTable() {
  ...
  },
  {
    ...
    confirmTitle: t("admin.productTable.bulkActions.publishTitle", { count: selectedRows.length }),
    confirmMessage: t("admin.productTable.bulkActions.publishMessage", { count: selectedRows.length }),
    isDisabled: selectedRows.length === 0,
    onClick: async () => {
    ...
    confirmTitle: t("admin.productTable.bulkActions.makeVisibleTitle", { count: selectedRows.length }),
    confirmMessage: t("admin.productTable.bulkActions.makeVisibleMessage", { count: selectedRows.length }),
    isDisabled: selectedRows.length === 0,
    onClick: async () => {
      const errors = [];
      const successes = [];
    ...
    confirmTitle: t("admin.productTable.bulkActions.makeHiddenTitle", { count: selectedRows.length }),
    confirmMessage: t("admin.productTable.bulkActions.makeHiddenMessage", { count: selectedRows.length }),
    isDisabled: selectedRows.length === 0,
    onClick: async () => {
      const errors = [];
      const successes = [];
    ...
    confirmTitle: t("admin.productTable.bulkActions.duplicateTitle", { count: selectedRows.length }),
    confirmMessage: t("admin.productTable.bulkActions.duplicateMessage", { count: selectedRows.length }),
    isDisabled: selectedRows.length === 0,
    onClick: async () => {
      const { data, error } = await apolloClient.mutate({
    ...
    label: t("admin.productTable.bulkActions.archive"),
    confirmTitle: t("admin.productTable.bulkActions.archiveTitle", { count: selectedRows.length }),
    confirmMessage: t("admin.productTable.bulkActions.archiveMessage", { count: selectedRows.length }),
    isDisabled: selectedRows.length === 0,
    onClick: async () => {
      const { data, error } = await apolloClient.mutate({
  ...
  const cardTitle = (
    <Fragment>
      {t("admin.products")}<span className={classes.selectedProducts}>{selectedProducts}</span>
    </Fragment>
  );
